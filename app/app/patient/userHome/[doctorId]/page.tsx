...
function DocDetails() {
    ...
    const fetchDoctorDetails = async () => {
      if (doctorId) {
        try {
          ...
          const q = query(
            doctorsRef,
            where("walletAddress", "==", doctorId as string)
          );
  ...
  const handleBooking = async () => {
    ...
    try {
      ...
      const platformFeeInLamports = Math.round(
        consultationFeeInSol * 0.1 * LAMPORTS_PER_SOL
      );
      const doctorFeeInLamports = Math.round(
        consultationFeeInSol * 0.9 * LAMPORTS_PER_SOL
      );
    ...
    <main className="w-11/12 max-w-lg mx-auto font-urbanist min-h-screen flex flex-col">
      ...
      <article>
        ...
        <p className="leading-5">
          Dr. {doctorDetails.firstName} {doctorDetails.lastName}{" "}
          {doctorDetails.about}...{" "}
          <span className="text-custom-blue hover-cursor">Read more</span>
        </p>
      </article>
